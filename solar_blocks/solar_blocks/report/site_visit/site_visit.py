...
def get_data(filters):
    ...
    filters={"custom_project_stage": "Site Visit", "project_status": ("!=", "Cancelled")}
)
	...
	total_no_of_jobs=0
	for project in data:
		task=frappe.db.get_all("Task",filters={'project':project.name,'subject':'Site Visit'},fields={'custom_name_completed_by','expected_close_date','custom_scheduled_on','description'})
		# frappe.throw(f"{task}")
		for task in task:
