...
def get_data(filters):
    ...
    filters={"custom_project_stage": "Engineering", "project_status": ("!=", "Cancelled")}
)
	...
	total_no_of_jobs=0
	for project in data:
		task=frappe.db.get_all("Task",filters={'project':project.name,'subject':'Engineering'},fields={'custom_name_completed_by','expected_close_date','description'})
		# frappe.throw(f"{task}")
		for task in task:
